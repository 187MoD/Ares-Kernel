...
# 5. Open Terminal, ‘cd’ to the Kernel ‘root’ folder and run ‘. build_variant-Aries.sh’
# 6. The output (anykernel zip) file will be generated in the ‘release_Aries’ folder
...
KERNEL_NAME="Aries-Kernel"

KERNEL_VARIANT="kltedv"	# options: klte, kltekor, kltedv, klteduos, kltekdi & all (build all the variants)

KERNEL_VERSION="alpha-v1.3"   # leave as such, if no specific version tag

KERNEL_DEFCONFIG="Aries_@$KERNEL_VARIANT@_defconfig"
...
ANYKERNEL_DIR="anykernel_Aries"

RELEASE_DIR="release_Aries"
...
if [ -z "$KERNEL_VARIANT" ]; then
	...
	make -C $(pwd) O=output_kltekor Aries_@kltekor@_defconfig
	# updating kernel version
	...
	make -C $(pwd) O=output_klte Aries_@klte@_defconfig
	# updating kernel version
	...
	make -C $(pwd) O=output_kltedv Aries_@kltedv@_defconfig
	# updating kernel version
	...
	make -C $(pwd) O=output_klteduos Aries_@klteduos@_defconfig
	# updating kernel version
	...
	make -C $(pwd) O=output_kltekdi Aries_@kltekdi@_defconfig
	# updating kernel version
